...
begin
  if not exists (
    ...
    create type public.user_role as enum ('admin', 'teacher', 'volunteer');
  end if;
  ...
  if not exists (
    ...
    create type public.announcement_audience as enum ('public', 'teachers', 'volunteers', 'admins');
  end if;
...
create table if not exists public.form_definitions (
  ...
  title text not null,
  description text,
  audience text not null check (audience in ('teachers', 'volunteers', 'public')),
